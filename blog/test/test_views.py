...
from blog.forms import CommentForm, EmailPostForm, PostForm, SearchForm
...
class PostModelTests(TestCase):
    ...
    def test_published_manager(self):
        ...
        Post.objects.create(
            title="Draft 1", body="Content", author=self.user, status=Post.Status.DRAFT
        )
        Post.objects.create(
            title="Published 2",
            body="Content",
...
class CommentModelTests(TestCase):
    ...
    def test_create_comment(self):
        ...
        comment = Comment.objects.create(
            post=self.post, commented_by=self.user, body="This is a great post!"
        )
    ...
    def test_comment_ordering(self):
        ...
        comment1 = Comment.objects.create(
            post=self.post, commented_by=self.user, body="First comment"
        )
        comment2 = Comment.objects.create(
            post=self.post, commented_by=self.user, body="Second comment"
        )
...
class PostListViewTests(TestCase):
    ...
    def test_post_list_out_of_range_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["posts"].number, 2
        )  # 5 posts, 3 per page = 2 pages
...
class PostDetailViewTests(TestCase):
    ...
    def test_post_detail_shows_comments(self):
        ...
        comment = Comment.objects.create(
            post=self.post, commented_by=self.user, body="Great post!", active=True
        )
    ...
    def test_post_detail_hides_inactive_comments(self):
        ...
        comment = Comment.objects.create(
            post=self.post, commented_by=self.user, body="Spam comment", active=False
        )
...
class PostUpdateViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.other_user = User.objects.create_user(
            username="otheruser", email="otheruser@email.com", password="otherpass123"
        )
        cls.post = Post.objects.create(
...
class PostCommentViewTests(TestCase):
    ...
    def test_add_comment_success(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        comment_data = {"body": "This is a great post!"}

        self.client.post(self.url, comment_data)
...
class PostSearchViewTests(TestCase):
    ...
    def test_search_trigram_similarity(self):
        ...
        self.assertGreater(len(response.context["results"]), 0)
        self.assertTrue(
            any("Django" in post.title for post in response.context["results"])
        )
...
class UserIsAuthorDecoratorTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.author = User.objects.create_user(
            username="author", email="testuser@email.com", password="authorpass"
        )
        cls.other_user = User.objects.create_user(
            username="other", email="other@email.com", password="otherpass"
        )
        cls.post = Post.objects.create(
            title="Author Post",
            body="Content",
